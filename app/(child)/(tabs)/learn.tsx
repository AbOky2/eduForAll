...
import { loadHomeSummary, type SubjectProgress } from '@/features/learning-path/application/home-summary';
...
const MODULE_META: Record<Subject, { label: string; icon: IconName; pastille: string; tint: string; halo: string }> = {
  reading: { label: fr.subjects.reading, icon: 'book', pastille: colors.secondaryContainer, tint: colors.onSecondaryContainer, halo: '#dcefff' },
  writing: { label: fr.subjects.writing, icon: 'pencil', pastille: colors.tertiaryFixed, tint: colors.onTertiaryContainer, halo: '#fdf3d8' },
  dictation: { label: fr.subjects.dictation, icon: 'ear', pastille: colors.primaryFixedDim, tint: colors.onPrimaryContainer, halo: '#f6e5cf' },
  math: { label: fr.subjects.math, icon: 'calculator', pastille: colors.secondary, tint: colors.onSecondary, halo: '#e3edf3' },
};
        ...
        {subjects.map((subject) => {
          ...
          return (
            ...
            >
              ...
              <View style={[styles.pastille, { backgroundColor: subject.locked ? colors.lockedContainer : meta.pastille }]}>
                <AlifaIcon name={meta.icon} size={26} color={subject.locked ? colors.locked : meta.tint} />
              </View>
              <View style={styles.moduleText}>
                <AlifaText variant="headlineMd">{meta.label}</AlifaText>
                <View style={styles.badge}>
                  <AlifaText variant="labelSm" color={colors.onSecondaryContainer}>
                    {subject.completed > 0 ? fr.home.lessonsDone(subject.completed) : fr.home.newBadge}
                  </AlifaText>
                </View>
              </View>
              <AlifaIcon
